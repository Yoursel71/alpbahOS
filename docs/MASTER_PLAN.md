...
| M04 | Chroot ve nihai LFS temel sistemi | M02 kararları, M03 | Kısmi: Coreutils, Zlib, Gzip, Zstd, Xz, Bzip2, Lz4, File, Readline ve M4 için izole stage manifestleri/preflight kanıtı var; bazıları rootfs ile uyuşmuyor ve tam temel paket kapsamı yok. M4 1.4.20 `make check` 306 PASS/60 SKIP/0 FAIL; stage smoke geçti, 36/93 kayıt eşleşti. Rootfs `alp` DB boş, base-package removal guard yok; kayıt eklemek güvenli değil. Hiçbir uyuşmayan stage birleştirilmedi. [Değerlendirme](verification/m04-base-ownership-assessment-2026-09-23.md). |
