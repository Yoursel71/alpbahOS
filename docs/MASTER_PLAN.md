...
| M05 | Kernel, init, bootloader ve ilk VM açılışı | M04 | Boot kabul kanıtı mevcut: BIOS ve UEFI yollarında hedef kernel'den giriş, ağ ve reboot doğrulandı. Resmî kapanış, §10.1'deki nihai temel sistem sahiplik kaydı koşulu nedeniyle M04 kapanana kadar beklemede. 24 Eyl 14:02 UTC canlı Gen2/UEFI kontrolü: `6.16.1-alpbahOS`, PARTUUID+`rootwait`, `/dev/sda2`, routable DHCP `172.28.164.164/20`, `resolvectl` DNS, zorunlu SSH-only public-key bağlantısı, etkin getty/resolved/sshd, 0 failed unit, host ping 4/4. 16:35 UTC recheck: admin tty1/seat0 `Service=login`, `Type=tty`, `Active=yes`; getty/resolved/sshd active and no failed units. [WORKLOG kanıtı](WORKLOG.md). |
