...
| M07 | Hazır masaüstü ve temel uygulama profili | M06 | P0 KWin unit/Wayland-only kapısı geçti; 24 Eyl 17:44 UTC canlı `loginctl` recheck'i `admin` session 27 `seat0/tty1`, `Service=login`, `Type=tty`, `Active=yes` ve PAM kaydını doğruladı. `plasma.desktop` ile `startplasma-wayland` mevcut; KWin inactive ve Plasma süreçleri yok. Gerçek Plasma oturumu, ayarlar ve dosya yöneticisi kanıtı hâlâ bekliyor. [Preflight](verification/m07-gen2-login-wayland-preflight-2026-09-24.log). |
