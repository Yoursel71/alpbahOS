...
| M04 | Chroot ve nihai LFS temel sistemi | M02 kararları, M03 | Kısmi: Coreutils, Zlib, Gzip, Zstd, Xz, Bzip2, Lz4, File, Readline, M4, Diffutils, Findutils ve Gawk için izole stage manifestleri/preflight kanıtı var; bazıları rootfs ile uyuşmuyor ve tam temel paket kapsamı yok. Gawk 5.3.2 `make check` ALL TESTS PASSED; awk smoke geçti, 8/160 kayıt eşleşti. Rootfs `alp` DB boş, base-package removal guard yok; kayıt eklemek güvenli değil. Hiçbir uyuşmayan stage birleştirilmedi. [Değerlendirme](verification/m04-base-ownership-assessment-2026-09-23.md). |
