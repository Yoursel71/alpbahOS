...
|---|---|---|---|
| Bağımlılık çözümü | Var, olgun | **Kısmen — Seviye 1+2 var (22 Eylül 2026)** | Sürüm kısıtı/çakışma çözümü YOK, dış (PyPI/apt tarzı) bir evrene açılmıyor — sıfırdan tam bağımlılık çözücü MASTER_PLAN tarafından zaten yasaklanmış. Var olanlar: (Seviye 1) `requires_commands`/`requires_libraries` ile eksik sistem aracı/kütüphanesi indirme/derleme denenmeden ÖNCE bildirilir; (Seviye 2) `depends` alanıyla **yalnızca bu kataloğun kendi içinde** zincirleme kurulum — `alp install X`, X'in kataloğumuzdaki bağımlılıklarını doğru sırada (topological sort, döngü tespitli) otomatik kurar. İkisi de gerçek Linux'ta uçtan uca doğrulandı. Ayrıntı: `006-alp-real-linux-test.md`, `007-alp-more-recipes-symlink-fix.md`, `008-alp-preflight-deps.md`, `009-alp-catalog-deps.md` |
...
| Mağaza (Discover) entegrasyon yolu | PackageKit alpm backend, kaynakta mevcut | **Kısmen var** — `--json` çıktı + `alp_*` sarmalayıcılar yazıldı/test edildi; gerçek `AlpPackageKitBackend` sınıfı yazıldı ama hiç PackageKit daemon'unda çalıştırılmadı | Bu, D11/AGENTS.md'nin GUI gereksinimini henüz tam karşılamaz. Ayrıntı: [design/packagekit-integration.md](alp-prototype/design/packagekit-integration.md) |
