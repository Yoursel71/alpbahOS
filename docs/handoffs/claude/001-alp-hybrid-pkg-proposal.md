...
| Yapılandırma korunması | Var (pacnew/pacsave) | **Yok, ama tasarımı yazıldı** | Prototip güncelleme/upgrade akışını hiç ele almıyor, sadece install/remove. Tasarım (pacnew/pacsave eşdeğeri `.alpnew`/`.alpsave`): [design/config-protection.md](alp-prototype/design/config-protection.md) — henüz kod değil |
...
| Mağaza (Discover) entegrasyon yolu | PackageKit alpm backend, kaynakta mevcut | **Yok, ama entegrasyon sözleşmesi yazıldı** | `alp` için PackageKit backend'i henüz yazılmadı; bu, D11/AGENTS.md'nin GUI gereksinimini henüz karşılamaz. Sözleşme: [design/packagekit-integration.md](alp-prototype/design/packagekit-integration.md) — henüz kod değil |
