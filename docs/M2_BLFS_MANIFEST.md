...
- PipeWire/WirePlumber oturumu Loopback kartını buldu; otomatik sink ve source ikisi de device 0'ı kullandı. `pw-play` → `pw-record` sessiz WAV aldı (peak/RMS 0); device 1'i PipeWire source olarak ekleme denemesi çalışan PCM node üretmedi. Sonraki bağımsız sanal-grafik testinde PipeWire `support.null-audio-sink` oluşturuldu; `pw-play` sink'e gönderildi ve monitor source `pw-record` ile yakalandı: 243712 kare, peak 12000, RMS 6522.08, SHA-256 `5c0547fd620ba0fd4e7128e9c11594d7128fa324ac450da0d63192263bc6e7b3`. PipeWire graph PCM I/O'su geçti; ALSA cihaz yönlendirmesi ve fiziksel ses kanıtı hâlâ yoktur.
...
| qtdeclarative | aynı dizindeki `qtdeclarative-everywhere-src-6.9.2.tar.xz` | MD5 `95cb51f5b8eb90feac825e31de555a4f` | Resmî `md5sums.txt` ile doğrulandı; 3237 hedef build/install ve QQuickView/QML offscreen smoke geçti |
| qtwayland | aynı dizindeki `qtwayland-everywhere-src-6.9.2.tar.xz` | MD5 `4e609abb0639482f109a23df84edd734` | Resmî `md5sums.txt` ile doğrulandı; 895 hedef build/install geçti. Configure: client, compositor, EGL ve DRM EGL = yes |
| libxkbcommon | `https://github.com/lfs-book/libxkbcommon/archive/v1.11.0/libxkbcommon-1.11.0.tar.gz` | MD5 `28be5e49d8aa19e20fd7acd4b24e19be` | X11 kapalı/Wayland odaklı build-install geçti; `pkg-config` ve `xkbcli --version` 1.11.0 doğrulandı; xkeyboard-config keymap data henüz yok |

Tam Qt BLFS meta build'i 47 GiB geçici alan gerektirdiğinden, Plasma için gereken modüller ayrı ayrı configure/build/install ediliyor. QtBase EGLFS/LinuxFB sunuyor, QtWayland ayrıca kuruldu; XCB/Xorg ve gerçek masaüstü oturumu henüz yok. QtWayland runtime klavye eşlemesi için `xkeyboard-config` verisi henüz yüklenmedi.
