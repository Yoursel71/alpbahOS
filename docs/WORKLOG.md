...
- **p11-kit 0.25.5:** İlk Meson koşusu 66/67; `common/test-path` SIGSEGV. Kök neden test ortamında hedef rootfs `/etc/passwd` içinde bulunmayan UID 1000/1001 ile çalıştırılması: `/path/expand` kullanıcı kaydı bulunamayınca testin korumasız davranıyor. 23 Eylül kontrollü karşılaştırmada aynı `common/test-path` ikilisi geçerli `tester` UID 101 ile 9/9 geçti, hedefte kaydı bulunmayan UID 1000 ile signal 11/exit 139 hatasını yeniden üretti. `trust list --filter=ca-anchors` 172 anchor döndürdü. Çökme hedefte bulunmayan build UID'leriyle test koşma ortamına özgü; trust-store arızasına dair kanıt yok. Birleşik log `/mnt/lfs/tmp/alp-logs/p11-kit-uid-compare.log`; ilk başarısız Meson logu `/mnt/lfs/tmp/alp-logs/p11-kit-build.log` içinde korunuyor.
