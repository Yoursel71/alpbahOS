...
- Gen1 kernel config incelemesinde `CONFIG_DRM_HYPERV` kapalı, `CONFIG_HYPERV=y`, `CONFIG_PCI=y`; Hyper-V Gen1 Wayland görüntü yolunu açmak için aynı Linux 6.16.1 kaynak ağacında `CONFIG_DRM_HYPERV=y` config hazırlandı (`CONFIG_DRM_SIMPLEDRM` yok). Kernel henüz derlenmedi/kurulmadı.
