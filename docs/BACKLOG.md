...
| VM-SSH-01 | Codex | Yeni test/release VM imajı | OpenSSH server ve anahtar tabanlı erişimi imaja ekle | `sshd` enabled/active; kullanıcı public key'i ile dış hosttan login doğrulanır | Yeni VM'lerde zorunlu; mevcut M2 adayında TCP/22 kapalı |
...
| DESKTOP-01 | Codex | BLFS-01, UI/SHELL girdileri | LFS içinde Plasma oturumu | Temiz kullanıcı, düşük kaynak ölçümü | Devam ediyor — KDE Frameworks, KWin 6.4.4 ve Plasma Workspace/Integration/Breeze kurulu; Hyper-V'de gerçek Wayland Plasma oturumu henüz doğrulanmadı |
