...
| VM-SSH-01 | Codex | Yeni test/release VM imajı | OpenSSH server ve anahtar tabanlı erişimi imaja ekle | `sshd` enabled/active; kullanıcı public key'i ile dış hosttan login doğrulanır | **Tamamlandı (23 Eylül 2026):** `alpbahOS-M2-SSH-Gen1`, TCP/22 ve `sa` public-key login hosttan doğrulandı. Artifact: `F:\alpbahOS-build\artifacts\alpbahOS-m2-ssh-gen1.vhdx`; `sshd` enabled/active, parola/root SSH kapalı. Sonraki yeni VM'lerde de bu erişim koşulu zorunlu. |
...
| BOOT-01 | Codex | BUILD-03 | Gen2 ve Gen1 boot | Hedef kernel ile giriş/ağ/reboot | M1 Gen2 geçti; M1 Gen1 DHCP/ping/reboot geçti; M2 adayı Gen1 login + guest DHCP (`172.28.162.13/20`) geçti, kullanıcı kontrollü reboot sonrası aynı MAC yeni `172.28.165.187` adresinde host ping 4/4, TTL 64 geçti |
...
| DESKTOP-01 | Codex | BLFS-01, UI/SHELL girdileri | LFS içinde Plasma oturumu | Temiz kullanıcı, düşük kaynak ölçümü | **Devam ediyor:** D-Bus user bus SSH'de doğrulandı. Plasma Workspace'in `--xwayland` başlatma argümanı KWin `KWIN_BUILD_X11=OFF` profiliyle çakışıyordu; patch ile `plasma_session` yeniden derlendi ve VM'e aktarıldı. SSH oturumu aktif DRM seat sağlamadığından gerçek masaüstü doğrulaması Hyper-V konsolundaki `sa` login oturumunda yapılmalı. |
