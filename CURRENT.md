...
| M07 — Plasma ve temel uygulamalar | Devam ediyor — konsol oturumu bekleniyor | SSH PAM `XDG_RUNTIME_DIR` ve user manager sağlıyor; D-Bus user bus socket etkin ve `org.freedesktop.systemd1` yanıt veriyor. Plasma Workspace'in KWin'e verdiği hatalı `--xwayland` argümanı kaynakta düzeltildi ve `plasma_session` yeniden derlenip kuruldu. `sa`'nın tty1 login kabuğu Plasma'yı otomatik başlatacak şekilde ayarlandı. 23 Eylül son SSH denetiminde tty1 `agetty` active, fakat `loginctl` yalnız SSH oturumlarını gösteriyor; gerçek DRM-seat oturumu ve Plasma henüz doğrulanmadı. Konsol audit gürültüsünü azaltmak için geçici kernel console loglevel 3'e çekildi ve tty1 getty yeniden başlatıldı. |
