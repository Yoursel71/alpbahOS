...
- **Qt/KWin:** QtBase ilk kurulumunda libxkbcommon olmadığı için `xkbcommon=no` idi. `FEATURE_xkbcommon=ON` yeniden derlemesi/kurulumu geçti; private header ve QXkbCommon sembolleri kuruldu. KWin Wayland/DRM 6.4.4 `KWIN_BUILD_X11=OFF`, `KWIN_BUILD_SCREENLOCKER=OFF` ile geçti; `kwin_wayland --version` doğrulandı. Gen1 kernel config'inde `CONFIG_DRM_HYPERV=y`, `CONFIG_DRM_FBDEV_EMULATION=y`, framebuffer console etkin ve `CONFIG_FB_HYPERV` kapalı; kernel henüz yeniden derlenmedi. `plasma5support` derleniyor.
