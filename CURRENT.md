...
| M07 — Plasma ve temel uygulamalar | Devam ediyor | KWin 6.4.4, Plasma Workspace, Plasma Integration ve Breeze QQC2 kuruldu. `plasmashell --version` offscreen ortamda 6.4.4 verdi; loader taramasında eksik dinamik bağımlılık yok. Wayland-only yamaları temiz BLFS arşivine karşı `patch --dry-run` ile doğrulandı. Gen1 DRM kernel'i derlenip ayrı aday VHDX'e kuruldu. F: artifact SHA-256 ile doğrulandı; read-only rootfs smoke testleri geçti. Ayrı Hyper-V guest boot ve gerçek Plasma oturumu bekliyor. |
