...
| M07 — Plasma ve temel uygulamalar | Devam ediyor — oturum doğrulaması bekliyor | SSH PAM `XDG_RUNTIME_DIR` ve user manager sağlıyor; D-Bus user bus socket etkin ve `org.freedesktop.systemd1` yanıt veriyor. İlk hata, Plasma Workspace'in Xwayland kapalı KWin'e `--xwayland` iletmesi olarak kaynakta doğrulandı. `docs/patches/plasma-workspace-6.4.4-wayland-no-xwayland.patch` ile `plasma_session` yeniden derlendi ve M2 VM'e kuruldu. SSH ile başlatılan KWin artık no-Xwayland yoluna giriyor; bu remote session aktif DRM seat'e sahip olmadığından gerçek Hyper-V konsolundaki kullanıcı oturumu bekleniyor. Plasma masaüstü henüz doğrulanmadı. |
