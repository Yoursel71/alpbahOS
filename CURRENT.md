...
| M05 — kernel/boot/VM | M1 Gen1 koşulları geçti; M2 adayı kısmi | Yeni M2 adayı Hyper-V Gen1'de login istemine ulaştı; guest `networkctl` DHCP lease'ini ve hosttan ping'i doğruladı. Kontrollü reboot sonrası yeniden erişim bekleniyor. |
...
- Kullanıcının guest `networkctl status eth0 --no-pager` ekran görüntüsü DHCPv4 `172.28.162.13/20`, gateway/DNS `172.28.160.1` değerlerini gösterdi; IPv4 geçidi `172.28.160.1` üzerinden alındı. Guest `ip -4 -brief address` çıktısı da `eth0` için `172.28.162.13/20` gösterdi. `sudo reboot` ve reboot sonrası yeni login/ping henüz bekliyor. KWin/Plasma oturumu da çalıştırılmadı; bu nedenle M05'in reboot koşulu ve M07 kapanışı açık kalır.
