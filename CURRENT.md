...
| M04 — nihai LFS tabanı | Kısmi; mevcut kurulumdan sahiplik kayıtları geriye dönük üretilemiyor | 23 Eylül Builder tekrar kontrolü: `/mnt/lfs/var/lib/alp/db.json` boş `packages`, `alp.py` SHA-256 beklenen `7b2998a5…`, `/` üzerinde 25 GiB boş, `/sys/block/nbd0` yok. Derin taramadaki `install_manifest.txt` dosyaları BLFS/desktop'a ait. Tam LFS-base manifestleri bulunmadı; güvenli kapanış için base paketlerini kontrollü yeniden kurup staging tree'lerini kaydetmek veya eşdeğer tarihsel kayıt bulmak gerekiyor. [Kanıt](docs/verification/m04-base-ownership-assessment-2026-09-23.md). |
| M05 — kernel/boot/VM | Tamamlandı (BIOS/UEFI boot, ağ ve reboot doğrulandı) | Geçmiş Gen1 BIOS ile güncel Gen2/UEFI boot/ağ/reboot kanıtları mevcut. Son Gen2 reboot sonrası kernel `6.16.1-alpbahOS`, boot ID `6df39e0d-a48d-4e8f-a138-6ec5c4e4a663`; `root=PARTUUID=84b8b2c9-9a5c-46b9-b9a0-0a22834278fe rootwait`, `/dev/sda2`; DHCP `172.28.164.164/20`, host ping 4/4, key-only SSH, boş failed-unit listesi. TTY1 PAM/seat oturumu M07'ye aittir ve hâlâ bekliyor. |
| M06 — BLFS altyapısı | Kısmi | M05 reboot sonrası canlı Gen2 guest'te DNS ve HTTPS/TLS (GitHub `curl -I`, HTTP 200) geçti. Önceki testler: PAM/logind, pkexec e2e, Mesa softpipe EGL çizim/readback, ALSA loopback ve PipeWire null-sink graph PCM roundtrip. Guest `aplay -l` bu VM'de ses kartı bulmadı; kullanıcı oturumunda grafik test ve gerçek Plasma session henüz yapılmadı. |
| M07 — Plasma ve temel uygulamalar | P0 KWin kapısı geçti; gerçek oturum bekliyor | Reboot sonrası `systemctl --user show/cat plasma-kwin_wayland.service`: FragmentPath `/usr/lib/systemd/user/plasma-kwin_wayland.service`, `/etc` Wayland-only drop-in yüklü; etkin ExecStart yalnız `/opt/kf6/bin/kwin_wayland_wrapper`, `--xwayland` yok. getty/resolved active, admin test hesabı mevcut. VMConnect tty1'de admin login ve `loginctl` `seat0`, `Type=tty`, active PAM/systemd session doğrulaması; ardından gerçek Plasma testi hâlâ yapılmadı. |
...
## 23 Eylül 2026 — SSH'li M2 sürümü ve Plasma kök neden düzeltmesi

- F: release: `F:\alpbahOS-build\artifacts\alpbahOS-m2-ssh-gen1.vhdx`; SHA-256 `fa29d47608a4444e043a3cbfa26753fd32a3c7e0b2078179dbcd852eab322c78`; 20 GiB dynamic standalone VHDX, parent yok. Aktif `alpbahOS-M2-SSH-Gen1` VM ayrı çalışma kopyasından açılıyor; release artifact çalışırken değişmiyor.
- Çalışan VM Gen1 / 2 vCPU / 2 GiB sabit RAM, MAC `00-15-5D-00-02-08`, güncel DHCP `172.28.169.177`. Host ping 4/4.
...
- **Açık tek adım:** gerçek DRM-seat testi için M2 VMConnect konsolunda `sa` ile normal login gerekli; kullanıcıdan komut istenmiyor. Login ekranına ulaşıldıktan sonra oturum testi ve M07 durumu güncellenecek.

## 23 Eylül 2026 — M05 Gen1 canlı DHCP/reboot tekrar doğrulaması

- Aktif VM `alpbahOS-M2-SSH-Gen1` Generation 1, MAC `00-15-5D-00-02-08`; SSH ile guest `networkctl status eth0 --no-pager` DHCPv4 `172.28.169.177/20`, gateway/DNS `172.28.160.1` gösterdi. Windows host `ping.exe -n 4 172.28.169.177`: 4/4, %0 kayıp, TTL 64.
...
## 23 Eylül 2026 — M04 yeniden denetimi ve M05 Gen2 reboot doğrulaması
...
- `systemctl --failed` boş; getty@tty1, resolved, sshd enabled/active; test-only admin UID 1001 ve `/etc/shadow` mode 0600. SSH ayarlarında parolalı/kbd-interactive/root girişi kapalı. `loginctl` yalnız SSH session'larını gösterdi; gerçek tty1 admin login ve PAM/logind `seat0` session hâlâ bekliyor, bu nedenle gerçek Plasma testi başlatılmadı. Ayrıntılı canlı kanıt `docs/WORKLOG.md` sonundadır.
