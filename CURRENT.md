...
- **Gerçek LFS `alp` testi:** `/mnt/lfs` chroot içinde htop 3.3.0 kur/list/remove döngüsü çalıştırıldı. İlk kurulumda dosya modu `0644` olduğu için test çalıştırmayı reddetti; `alp.py` kopyalayıcısında `shutil.copyfile()` → `shutil.copy2()` düzeltmesiyle tekrar kuruldu: mod `0755`, `htop --version` 3.3.0, kaldırma sonrası dosya yok ve paket DB boş. Düzeltme Builder kopyasında sınandı; Claude'un kanonik özel repo'ya henüz aktarılmadı. Yama WORKLOG'da.
- **Qt/KWin engeli:** KWin'in `inputmethod.cpp` ve `xkb.cpp` derlemesi `qxkbcommon_p.h` bulamadı. QtBase ilk kurulumda libxkbcommon henüz olmadığı için `xkbcommon=no` yapılandırılmıştı. BLFS libxkbcommon 1.11.0 kurulu iken QtBase 6.9.2 `FEATURE_xkbcommon=ON` ile yeniden yapılandırıldı (`xkbcommon=yes`); tam rebuild devam ediyor. Gen1 kernel config'ine `CONFIG_DRM_HYPERV=y`, `CONFIG_DRM_FBDEV_EMULATION=y` ve framebuffer console eklendi, `CONFIG_FB_HYPERV` kapalı; kernel henüz yeniden derlenmedi. KWin ve gerçek Plasma oturumu halen açık.
