...
# LFS 12.4-systemd Expat-2.7.1 isolated package stage. Run on the Builder.

LFS=/mnt/lfs
BUILD=/mnt/lfs/build/expat-2.7.1-m04-r1
STAGE_HOST=/mnt/lfs/tmp/alp-m04-expat-stage-r1
STAGE_CHROOT=/tmp/alp-m04-expat-stage-r1
SOURCE=/mnt/lfs/sources/expat-2.7.1.tar.xz
LOG=/mnt/lfs/tmp/alp-logs/m04-expat-2.7.1-stage-20260924-r1.log
FIXTURE=/mnt/lfs/tmp/alp-m04-expat-smoke-20260924.xml
EXPECTED_MD5=9f0c266ff4b9720beae0c6bd53ae4469
EXPECTED_SHA256=354552544b8f99012e5062f7d570ec77f14b412a3ff5c7d8d0dae62c0d217c30
...
[[ ! -e "$BUILD" && ! -e "$STAGE_HOST" && ! -e "$LOG" && ! -e "$FIXTURE" ]]
...
chroot --userspec=101:101 "$LFS" /usr/bin/env -i \
    ...
    /bin/bash --noprofile --norc -c 'cd /build/expat-2.7.1-m04-r1 && make check'
chroot "$LFS" /usr/bin/env -i PATH=/usr/bin:/usr/sbin:/bin:/sbin \
    /bin/bash --noprofile --norc -c '
        cd /build/expat-2.7.1-m04-r1
        make DESTDIR=/tmp/alp-m04-expat-stage-r1 install
        install -v -m644 doc/*.{html,css} /tmp/alp-m04-expat-stage-r1/usr/share/doc/expat-2.7.1/
    '

printf '<alpbahOS-smoke/>' > "$FIXTURE"
chroot "$LFS" /usr/bin/env -i PATH=/usr/bin:/usr/sbin:/bin:/sbin \
    LD_LIBRARY_PATH="$STAGE_CHROOT/usr/lib" \
    "$STAGE_CHROOT/usr/bin/xmlwf" /tmp/alp-m04-expat-smoke-20260924.xml
[[ -s "$STAGE_HOST/usr/lib/libexpat.so" || -L "$STAGE_HOST/usr/lib/libexpat.so" ]]
echo 'EXPAT_STAGE_SMOKE=passed'
[[ $(sha256sum "$LFS/usr/lib/alp/alp.py" | awk '{print $1}') == "$ALP_SHA256" ]]
grep -q '"packages": {}' "$LFS/var/lib/alp/db.json"
[[ ! -s /sys/block/nbd0/pid ]]
FINISHED=1
