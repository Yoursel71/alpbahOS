...
# LFS 12.4-systemd Gperf-3.3 isolated package stage. Run on the Builder.

LFS=/mnt/lfs
BUILD=/mnt/lfs/build/gperf-3.3-m04-r1
STAGE_HOST=/mnt/lfs/tmp/alp-m04-gperf-stage-r1
STAGE_CHROOT=/tmp/alp-m04-gperf-stage-r1
SOURCE=/mnt/lfs/sources/gperf-3.3.tar.gz
LOG=/mnt/lfs/tmp/alp-logs/m04-gperf-3.3-stage-20260924-r1.log
EXPECTED_MD5=31753b021ea78a21f154bf9eecb8b079
EXPECTED_SHA256=fd87e0aba7e43ae054837afd6cd4db03a3f2693deb3619085e6ed9d8d9604ad8
...
chroot --userspec=101:101 "$LFS" /usr/bin/env -i \
    ...
    /bin/bash --noprofile --norc -c 'cd /build/gperf-3.3-m04-r1 && make check'
chroot "$LFS" /usr/bin/env -i PATH=/usr/bin:/usr/sbin:/bin:/sbin \
    /bin/bash --noprofile --norc -c '
        cd /build/gperf-3.3-m04-r1
        make DESTDIR=/tmp/alp-m04-gperf-stage-r1 install
    '
rm -f "$STAGE_HOST/usr/share/info/dir"
chroot "$LFS" "$STAGE_CHROOT/usr/bin/gperf" --version | head -n 1
echo 'GPERF_STAGE_SMOKE=passed'
[[ $(sha256sum "$LFS/usr/lib/alp/alp.py" | awk '{print $1}') == "$ALP_SHA256" ]]
grep -q '"packages": {}' "$LFS/var/lib/alp/db.json"
[[ ! -s /sys/block/nbd0/pid ]]
FINISHED=1
