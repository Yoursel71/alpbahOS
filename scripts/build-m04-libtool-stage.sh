...
# LFS 12.4-systemd Libtool-2.5.4 isolated package stage. Run on the Builder.

LFS=/mnt/lfs
BUILD=/mnt/lfs/build/libtool-2.5.4-m04-r1
STAGE_HOST=/mnt/lfs/tmp/alp-m04-libtool-stage-r1
STAGE_CHROOT=/tmp/alp-m04-libtool-stage-r1
SOURCE=/mnt/lfs/sources/libtool-2.5.4.tar.xz
LOG=/mnt/lfs/tmp/alp-logs/m04-libtool-2.5.4-stage-20260924-r1.log
EXPECTED_MD5=22e0a29df8af5fdde276ea3a7d351d30
EXPECTED_SHA256=f81f5860666b0bc7d84baddefa60d1cb9fa6fceb2398cc3baca6afaa60266675
...
chroot --userspec=101:101 "$LFS" /usr/bin/env -i \
    ...
    /bin/bash --noprofile --norc -c 'cd /build/libtool-2.5.4-m04-r1 && make check'
chroot "$LFS" /usr/bin/env -i PATH=/usr/bin:/usr/sbin:/bin:/sbin \
    /bin/bash --noprofile --norc -c '
        cd /build/libtool-2.5.4-m04-r1
        make DESTDIR=/tmp/alp-m04-libtool-stage-r1 install
    '
rm -f "$STAGE_HOST/usr/lib/libltdl.a"
chroot "$LFS" "$STAGE_CHROOT/usr/bin/libtool" --version | head -n 1
chroot "$LFS" "$STAGE_CHROOT/usr/bin/libtoolize" --version | head -n 1
[[ -s "$STAGE_HOST/usr/lib/libltdl.so" || -L "$STAGE_HOST/usr/lib/libltdl.so" ]]
echo 'LIBTOOL_STAGE_SMOKE=passed'
[[ $(sha256sum "$LFS/usr/lib/alp/alp.py" | awk '{print $1}') == "$ALP_SHA256" ]]
grep -q '"packages": {}' "$LFS/var/lib/alp/db.json"
[[ ! -s /sys/block/nbd0/pid ]]
FINISHED=1
